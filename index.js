...
const addNewTask = () => {
  ...
  const taskDragger = document.createElement('div');
  const id = generateID(10);
  taskDragger.innerHTML = getTaskTemplate(inputElement.value, id);
  document.getElementById('taskList').append(taskDragger.firstChild);
...
const restoreTasks = (element, key) => {
  ...
  const taskListTemplates = taskList.map((task) => getTaskTemplate(task.value, task.id));
...
const getTaskTemplate = (value, id) => {
  return `<div class="task plainTask" id="${id}">
            <div class="taskName">
              <div class="radio leftElement"><img width="15" src="/img/check.png"></div>
...
const onRadioClick = (event) => {
  const taskElement = event.target.closest('.plainTask');
  if (taskElement) {
...
const stringConstants = {
  ...
  localStorageDoneKey: 'doneTaskList'
}
